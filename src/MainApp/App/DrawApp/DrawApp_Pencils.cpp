...
void DrawApp::loadPencils() {
    ...
    for (const auto &entry : std::filesystem::directory_iterator(path)) {
        if (entry.path().extension() == ".so") {
            try {
                auto lib = dlLoader<Pencil_I>::getLib(entry.path().string(),
                    "createPencil", "destroyPencil");
                if (lib)
                    addPencil(lib);
            } catch (const std::exception &e) {
                LOG_ERROR(std::string("Failed to load pencil: ") + e.what());
            }
        }
    }
}
...
void DrawApp::addPencil(std::shared_ptr<Pencil_I>& pencil) {
    ...
    btn->setPosition(10, static_cast<float>(pencilsButtons.size() + 1) * 50);
